...
contract HUBTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, HUBR }
    ...
    address public HUBR;
    uint public HUBRQueue; // Delays change to HUBR address
    ...
    /**
        @notice allow approved address to deposit an asset for HUB
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint HUB needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( HUB ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn HUB for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IHUBERC20( HUB ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( HUBR ).balanceOf( msg.sender ); // Can only borrow against HUBR held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with HUB
        @param _amount uint
     */
    function repayDebtWithHUB( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IHUBERC20( HUB ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, HUB, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( HUB ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( HUB ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns HUB valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match HUB decimals
            value_ = _amount.mul( 10 ** IERC20( HUB ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.HUBR ) { // 9
            HUBRQueue = block.number.add( blocksNeededForQueue );
        } else return false;
