...
interface IPresaleEverlast {
    function mint(address account_, uint256 amount_) external;
}

contract EverlastPresale is Ownable {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    struct UserInfo {
        uint256 amount; // Amount BUSD deposited by user
        uint256 debt; // total EVER claimed thus pEVER debt
        bool claimed; // True if a user has claimed EVER
    }
    ...
    // Tokens to raise (BUSD) and for offer (pEVER) which can be swapped for (EVER)
    IERC20 public BUSD; // for user deposits
    IERC20 public pEVER;
    IERC20 public EVER;

    uint256 public price = 20 * 1e18; // 20 BUSD per pEVER

    uint256 public cap = 1000 * 1e18; // 1000 BUSD cap per user

    uint256 public totalRaisedBUSD; // total BUSD raised by sale

    uint256 public totalDebt; // total pEVER and thus EVER owed to users
    ...
    bool public claimPresale; // true when pEVER is claimable

    bool public contractPaused; // circuit breaker

    mapping(address => UserInfo) public userInfo;
    mapping(address => uint256) public everClaimable; // amount of EVER claimable by address
    ...
    /**
     *  @notice set EVER Token
     *  @param _EVER: address of EVER
     */
    function setEVER(address _EVER) external onlyOwner {
        require( _EVER != address(0) );
        EVER = IERC20(_EVER);
    }

    /**
     *  @notice set pEVER Token
     *  @param _pEVER: address of pEVER
     */
    function setPEVER(address _pEVER) external onlyOwner {
        require( _pEVER != address(0) );
        pEVER = IERC20(_pEVER);
    }

    /**
     *  @notice set price for pEVER
     *  @param _price: price for pEVER
     */
    ...
    // @notice lets users claim EVER
    // @dev send sufficient EVER before calling
    function claimUnlock() external onlyOwner {
        require(ended, "Sale has not ended");
        require(!claimable, "Claim has already been unlocked");
        require(EVER.balanceOf(address(this)) >= totalDebt, 'not enough EVER in contract');
        claimable = true;
        emit ClaimUnlocked(block.number);
    }


    // @notice lets users claim pEVER
    ...
    function deposit(uint256 _amount) external checkIfPaused {
        ...
        uint256 payout = _amount.mul(1e18).div(price).div(1e9); // pEVER to mint for _amount
        ...
        IPresaleEverlast( address(pEVER) ).mint( msg.sender, payout );

        emit Deposit(msg.sender, _amount);
    }
    
    /**
     *  @notice it deposits pEVER to withdraw EVER from the sale
     *  @param _amount: amount of pEVER to deposit to sale (9 decimals)
     */
    function withdraw(uint256 _amount) external checkIfPaused {
        require(claimable, 'EVER is not yet claimable');
        ...
        pEVER.safeTransferFrom( msg.sender, address(this), _amount );

        EVER.safeTransfer( msg.sender, _amount );

        emit Mint(address(pEVER), msg.sender, _amount);
        emit Withdraw(address(EVER), msg.sender, _amount);
    }
    ...
    // @notice it claims pEVER back from the sale
    function claimPresaleEVER() external checkIfPaused {
        require(claimPresale, 'pEVER is not yet claimable');
        ...
        pEVER.safeTransfer( msg.sender, payout );

        emit Withdraw(address(pEVER),msg.sender, payout);
    }
